...
def draw_seq(img, seq, rect, range_y=(None, None), color=(0, 0, 0), thickness=5):
    low, high = range_y
    ...
    cv2.polylines(
        img, pointlist, False, color, thickness=thickness, lineType=cv2.LINE_AA
    )
...
def draw_axis_y(img, rect, range_y, label, num_ticks=5, color=(0, 0, 0), thickness=5):
    left, right, top, bottom = rect
    height = bottom - top
    ...
    for t in ticks:
        ...
        cv2.line(img, (left_start - 5, y), (left_start, y), color, thickness=thickness)
        cv2.putText(
            img,
            lab,
            (left_start - 30 - w // 2, y + baseline),
            font_face,
            0.9,
            color,
            thickness=2,
...
def get_plotting_params(caps_2d, cap_3d, ang_names):
    ...
    height_total = (
        ...
        + height_3d
        + spacing_videos
        + height_angle * n_angles
        + spacing_angle * (n_angles + 1)
    )
...
def get_start_image(pp, ang_names):
    ...
    for angnum, name in enumerate(ang_names):
        start_y = (
            pp["start_angles"] + (pp["height_angle"] + pp["spacing_angle"]) * angnum
        )
        ...
        draw_axis_y(start_img, rect, (0, 180), "Angle", num_ticks=3, thickness=4)
...
def draw_data(start_img, frames_2d, frame_3d, all_angles, pp):
    ...
    frames_2d_resized = [
        cv2.resize(f, (w, height_2d)) for f, w in zip(frames_2d, widths_2d)
    ]
    ...
    for angnum, angles in enumerate(all_angles):
        ...
        draw_seq(imout, angles, rect, range_y=(0, 180), color=data_color, thickness=2)
        x = (left + right) // 2
        cv2.line(imout, (x, top + 15), (x, bottom - 15), indicator_color, thickness=2)
...
def visualize_combined(config, angle_fname, fnames_2d, fname_3d, out_fname):
    ...
    for name, angles in ang_values.items():
        ang_values_padded[name] = np.pad(
            angles, pad_size, mode="constant", constant_values=np.nan
        )

    fourcc = cv2.VideoWriter_fourcc(*"XVID")
    writer = cv2.VideoWriter(
        out_fname, fourcc, fps, (pp["width_total"], pp["height_total"])
    )
...
def process_session(config, session_path):
    ...
    vid_fnames_2d = glob(
        os.path.join(session_path, pipeline_videos_labeled_2d, "*.avi")
    )
    ...
    angle_fnames = glob(os.path.join(session_path, pipeline_angles, "*.csv"))
    ...
    for angle_fname in angle_fnames:
        ...
        if (
            ...
            and abs(get_nframes(out_fname) - get_data_length(angle_fname)) < 100
        ):
            continue
        ...
        visualize_combined(
            config, angle_fname, fnames_2d_current, fname_3d_current, out_fname
        )
