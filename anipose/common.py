...
def process_all(config, process_session, **args):
    ...
    next_folders = [
        (os.path.join(pipeline_prefix, folder), (folder,), level) for folder in folders
    ]
    ...
    while len(q) != 0:
        ...
        if nesting < 0:
            ...
            next_folders = [
                (os.path.join(path, folder), past_folders + (folder,), level + 1)
...
def find_calibration_folder(config, session_path):
    ...
    while level >= 0:
        ...
        if len(videos) > 0 or (len(intrinsics) > 0 and len(extrinsics) > 0):
            return curpath
...
def get_calibration_board(config):
    ...
    if board_type in ["aruco", "charuco"]:
        ...
        raise ValueError(
            ...
            "'aruco', 'charuco', or 'checkerboard' not '{}'".format(board_type)
        )
