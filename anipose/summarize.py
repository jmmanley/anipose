...
def get_angle_fnames(config, session_path):
    fnames = glob(os.path.join(session_path, config["pipeline"]["angles"], "*.csv"))
    return fnames


def get_pose3d_fnames(config, session_path):
    fnames = glob(os.path.join(session_path, config["pipeline"]["pose_3d"], "*.csv"))
    return fnames


def get_pose2d_fnames(config, session_path):
    fnames = glob(os.path.join(session_path, config["pipeline"]["pose_2d"], "*.h5"))
    return fnames


def get_pose2d_filtered_fnames(config, session_path):
    fnames = glob(
        os.path.join(session_path, config["pipeline"]["pose_2d_filter"], "*.h5")
    )
    return fnames
...
def make_summarize_fun(get_fnames_session, output_fname, h5=False):
    def summarize_fun(config):
        ...
        outdir = os.path.join(config["path"], config["pipeline"]["summaries"])
...
summarize_pose2d_filtered = make_summarize_fun(
    get_pose2d_filtered_fnames, "pose_2d_filtered.csv", h5=True
)
